...
def main(c_class):
    ...
    test_dataset = CorruptedCIFAR10C(data_path=CIFAR10_C_DATA_PATH, labels_path=CIFAR10_C_LABELS_PATH,
    ...
    # Testing function
    def test(model, loader, criterion, epoch, dataset_name='Test'):
        model.eval()
        running_loss = 0.0
        correct = 0
        total = 0
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        cifar10c_loss, cifar10c_acc = test(model, cifar10c_loader, criterion, epoch, dataset_name='CIFAR10-C')
    ...
    with open(root[:-5]+'corruption_accuracies_23_06.txt','a') as f:
        f.write(f'corruption accuracy, {corrupt_cls}, test accuracies {max(test_accuracies)}, train accuracies, {max(train_accuracies)}\n')
    ...
    plt.tight_layout()
    plt.savefig(str(corrupt_cls)+'23_06.png')

...
from engine2 import evaluate, train_one_epoch, train_and_validate
...
def main(args):
    ...
    dataset_val, num_classes = get_dataset(is_train=False, args=args)
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset_train, batch_sampler=train_batch_sampler, num_workers=args.workers, collate_fn=train_collate_fn
    )
    
    val_loader = torch.utils.data.DataLoader(
        dataset_val, batch_sampler=train_batch_sampler, num_workers=args.workers, collate_fn=train_collate_fn
    )
    ...
    model_resnet = torch.load('test_distilled_model_resnet50.pth')
    ...
    #     train_one_epoch(model, optimizer, train_loader, device, epoch, args.print_freq, scaler)
    train_and_validate(model, train_loader, val_loader, optimizer, device, args.epochs, args.print_freq,

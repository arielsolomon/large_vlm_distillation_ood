...
wandb.init(project='nano_Ex_ob_distilation_'+date)  # Replace with your project name
...
# Load data
src = '/work/large_vlm_distillation_ood/datasets/coco/10p_coco/'  # Adjust path if needed
...
def trainer(student, teacher, trainloader, epochs, lr,temperature):
    ...
    for epoch in range(epochs):
        for images, labels in tqdm(trainloader):
            ...
            wandb.log({"student_detection_loss": detection_loss})
            ...
            total_loss = (kl_loss+ detection_loss/loss_weight).sum()
            ...
            if epoch % 25 == 0:
                torch.save(student, f'distilled_nano_{epoch}.pt')
...
torch.save(trained_student_model, cwd+'/distiled_student_20_08.pt')
df = pd.DataFrame(losses)
df_s_losses = pd.DataFrame(s_detection_losses)
df_s_losses.to_csv(cwd+'/S_losses_'+date+'.csv', header=False)
df.to_csv(cwd+'/Total_losses'+date+'.csv', header=False)
